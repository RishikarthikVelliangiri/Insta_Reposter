...
# Configure stdout/stderr to handle Unicode properly right at the start
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                print(f"clip_upload failed: {str(e).replace('\ufffd', '?')}")
                ...
                try:
                    ...
                    print(f"video_upload also failed: {str(inner_e).replace('\ufffd', '?')}")
                    raise inner_e
        except Exception as e:
            print(f"Upload failed: {str(e).replace('\ufffd', '?')}")
        ...
        print(f"Unexpected error during repost process: {str(e).replace('\ufffd', '?')}")
        print("STEP_MARKER: UPLOAD_FAILED")
        return False
    finally:
...
if __name__ == "__main__":
    ...
    # Print arguments safely without emojis
    try:
        print("Received arguments:")
        print(f"URL: {args.url}")
        print(f"Source: {args.source}")
        print("Caption and hashtags received (not showing content to avoid encoding issues)")
    except Exception as e:
        print(f"Error printing arguments: {str(e).replace('\ufffd', '?')}")
    ...
    if args.url:
        try:
            ...
            print(f"Unexpected error: {str(e).replace('\ufffd', '?')}")
            print("STEP_MARKER: UPLOAD_FAILED")  # Clear marker for failure
            print(format_api_result(False, f"Error: Unicode safely removed"))
    else:
